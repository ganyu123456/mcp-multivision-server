...
Accepts four input forms for images (and, where noted, videos):
...
Provides helpers to (a) resolve an image into an ``image_url`` value for a VLM
request and (b) materialize any input into a real local file path for OpenCV /
ffmpeg processing.
"""
...
async def materialize(src: str, work_dir: str, *, suffix: str = "") -> tuple[str, bool]:
    """Ensure the input exists as a local file path.

    Returns (path, is_temp). When is_temp is True the caller should delete the
    file after use. Local paths / file:// URLs are returned as-is (is_temp=False).
    http(s) URLs, data URIs and base64 are written into work_dir.
    """
    src = src.strip()

    # 已存在的本地文件（含 file://）直接返回，不复制
    path = existing_path(src) if not is_http_url(src) and not is_data_uri(src) else None
    if path:
        return path, False
    ...
    if is_http_url(src):
        data = await fetch_bytes(src)
        if not suffix:
            suffix = os.path.splitext(urlparse(src).path)[1] or ".bin"
    elif is_data_uri(src) or looks_like_base64(src):
        data = decode_base64(src)
    else:
        raise InputError(f"file not found: {_normalize_path(src)}")

    os.makedirs(work_dir, exist_ok=True)
    fd, path = tempfile.mkstemp(dir=work_dir, suffix=suffix)
